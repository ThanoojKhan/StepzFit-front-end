                        ...
                        <ul className="">
                            <li className="border w-full text-white border-gray-300 p-4 rounded-md shadow-md flex justify-between items-center">
                                <div className="flex-grow">
                                    <p className="text-lg font-semibold">Task: {item.task}</p>
                                    <p className="">Status: {item.status}</p>
                                    <p className="mt-2">Some other data: {item.someOtherData}</p>
                                </div>
                                <span className="text-green-600">Status: {item.status}</span>
                            </li>
                        </ul>
                    </div>
                ))
            ) : (
                <p className="text-white mt-5">No data to show</p>
            )}
        </div>
    );
}
