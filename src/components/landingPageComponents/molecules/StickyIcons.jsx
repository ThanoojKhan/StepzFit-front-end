...
const StickyIcons = () => {
    ...
    return (
        <>
            {isOpen ?
                <aside className={`fixed lg:bottom-0 bottom-1/2 left-0 flex flex-col gap-5 items-center bg-gray-800 rounded-e-lg py-3 px-2 ${isOpen ? "" : "hidden"
                    }`}
                >
                ...
                <aside className={`fixed lg:bottom-0 bottom-1/2 left-0 flex flex-col gap-5 items-center bg-gray-800 rounded-e-lg py-3 px-2 ${isOpen ? "" : "hidden"}`}>
                    <p onClick={toggleIcons} className="text-zinc-100 -mt-10 cursor-pointer hover:text-zinc-900">Follows us</p>
                </aside>
            }
        </>
